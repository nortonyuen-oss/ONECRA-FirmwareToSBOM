...
./fw2sbom.py --batch <資料夾> [-d 輸出資料夾] [--recursive]
```
...
| `--version` | 顯示工具版本 |
| `--batch DIR` | 分析 DIR 裡的每個檔案(見下方〈批次分析〉) |
| `--recursive` | 搭配 `--batch`,包含子資料夾 |
| `--no-evidence` | 搭配 `--batch`,不產生 Excel 證據報告 |

**Exit codes**: `0` 成功、`1` 輸入/IO/參數錯誤、`2` `--fail-if-empty` 且無元件命中、
`3` `--batch` 時有檔案沒有產生 SBOM。

### 批次分析

CRA 要的是**持續、可重跑**的 SBOM 流程,不是一次性的報告。一個產品線有幾十個韌體
版本,逐個拖是不實際的:
...
- 每個檔案各自產生 `<名稱>_SBOM.cdx.json`、`<名稱>_SBOM.spdx.json` 與
  `<名稱>_Evidence.xlsx`(批次預設兩種格式都出;`--format` 可改)。
- 另外寫一份 **`batch-summary.csv`**(Excel 直接打開)與 `batch-summary.json`:
  每個檔案的 SHA-256、大小、格式、架構、發行版、元件數、無法分析的區段數、
  耗時、輸出檔名,以及失敗原因。
- **一個檔案失敗不影響其他檔案。** 失敗寫進摘要,exit code 為 `3`,讓 CI 分得出
  「全部成功」與「去看摘要」。
- **同名檔案不會互相覆蓋**:`a/fw.bin` 與 `b/fw.bin` 會得到不同的輸出名。
- 預設輸出到 `<資料夾>/fw2sbom-output/`;重跑時**不會把上一次的輸出當成韌體**。
- 一次分析一個檔案,記憶體用量不會因為檔案多而增加。
...
可以一次選取多個檔案、拖入整個資料夾,或按「或選擇整個資料夾」。只有一個檔案時
介面與以前完全相同;多個檔案時會出現一張批次表:

- 檔案**依序**分析,每一列顯示等待中 / 分析中 / 完成 / 錯誤、元件數、無法分析的
  區段數,以及三個下載連結;按「查看」在下方顯示那個檔案的完整結果。
- **「全部下載(ZIP)」**把每個檔案的 CycloneDX、SPDX、證據報告,加上一份
  `batch-summary.csv`,打包成一個 zip。同名檔案會自動改名(`router (2)_SBOM.cdx.json`),
  不會互相覆蓋。
- 廠商 SBOM 只用於單一檔案分析:把一份供應商 SBOM 拿去和一整批不同的韌體比對
  沒有意義,批次時不會套用,頁面會說明。

### 大檔案

- **上傳一邊到達一邊解析**,韌體在記憶體裡只有一份。以前會先把整個請求讀進記憶體
  再切開,一份 500 MB 的 flash dump 在分析開始前就佔用四、五倍的記憶體;實測 100 MiB
  的上傳,尖峰由約 400 MiB 降到約 107 MiB。
- **仍然不寫入磁碟** —— 暫存檔也是客戶韌體的一份拷貝,放在一個他們沒有選擇的地方。
- **一次只跑一個分析。** 第二個上傳(另一個分頁、批次裡的下一個檔案)會排隊,
  進度條顯示「排隊等候其他分析完成」。兩份 500 MB 的映像同時展開,是筆電記憶體
  用完的典型方式。
- 分析結果保留一小時,上限改以**總位元組數**計(512 MB),不再只算筆數 —— 五十個
  小型 MCU 結果不會把批次的第一個擠掉。
